...
def test_return_after_checkout(idle_asset_id):
    runner.invoke(
        app, ["asset", "checkout", idle_asset_id, "--to-holder", "X", "--json"]
    )
    res = runner.invoke(
        app,
        [
...
def test_report_broken_keeps_holder_by_default(idle_asset_id):
    ...
    runner.invoke(
        app, ["asset", "checkout", idle_asset_id, "--to-holder", "A", "--json"]
    )
...
def test_report_broken_explicit_clear_holder(idle_asset_id):
    ...
    runner.invoke(
        app, ["asset", "checkout", idle_asset_id, "--to-holder", "B", "--json"]
    )
    res = runner.invoke(
        app, ["asset", "report-broken", idle_asset_id, "--to-holder", "", "--json"]
    )
    assert res.exit_code == 0, res.stdout
    body = json.loads(res.stdout)
    assert body["data"]["to_holder"] is None
...
def test_dismiss_keeps_holder(idle_asset_id):
    ...
    runner.invoke(
        app, ["asset", "checkout", idle_asset_id, "--to-holder", "C", "--json"]
    )
...
def test_transfer_holder_command_removed(idle_asset_id):
    res = runner.invoke(
        app, ["asset", "transfer-holder", idle_asset_id, "--to-holder", "Y"]
    )
