...
def test_probe_returns_ok_on_first_success():
    ...
    with (
        patch("asset_hub.probe._open", return_value=fake_response),
        patch("asset_hub.probe.time.sleep"),
    ):
...
def test_probe_succeeds_mid_loop():
    ...
    with (
        patch("asset_hub.probe._open", side_effect=side_effects),
        patch("asset_hub.probe.time.sleep"),
    ):
        result = probe_until_ready("http://x/healthz")
    assert result.ok is True
...
def test_probe_once_returns_true_on_200():
    ...
    fake_response.__exit__.return_value = False
    fake_response.status = 200
    with patch("asset_hub.probe._open", return_value=fake_response):
        assert probe_once("http://x/healthz") is True


def test_probe_once_returns_false_on_error():
    with patch("asset_hub.probe._open", side_effect=URLError("x")):
