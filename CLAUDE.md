...
桌面便携版构建：`uv sync --extra desktop` + `pnpm --dir frontend build` + `uv run pyinstaller packaging/asset_hub.spec`。产物在 `dist/asset-hub/`，CI 归档为 zip。详见 `packaging/asset_hub.spec` 和 `references/deploy.md`。
...
- `desktop/` — 桌面便携版第三接口层（pywebview WebView2 内嵌），入口 `desktop/main.py`
