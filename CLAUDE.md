...
- 数据库迁移：`uv run alembic revision --autogenerate -m "<slug>"` 生成 + `uv run alembic upgrade head` 执行。`alembic/env.py` 用 `Settings()` 覆盖 `sqlalchemy.url`（默认 `data/asset_hub.db`），并启用 `render_as_batch=True`（SQLite 改表必需）——新写迁移别绕过 batch。
...
- 不写 e2e；UI 视觉/交互改动**优先**用 `playwright` MCP 自动化烟测（`browser_navigate` / `browser_snapshot` / `browser_click` / `browser_fill_form` / `browser_take_screenshot` 等），覆盖不到的场景再退到让用户手动跑 dev server 验证。
