...
- [ ] **Step 2: 同时注册 `--color-warning` token 在 CSS variable + Tailwind utility 两层（F2 修订）**

**关键原则**：仅加 CSS variable 是不够的——`bg-warning/10` / `text-warning` / `border-warning/30` 这些 Tailwind utility 必须在 `@theme` 段或 tailwind.config 里注册才会生成对应 utility class。Task 34 banner 同时用 utility 与 inline style 是双轨混用必踩坑（plan review F2）。

修改 `frontend/src/styles/globals.css`：

```css
/* 1. raw HSL 给 inline style 兜底（与 --color-cta 一致的写法）*/
:root {
  --color-warning-hsl: 38 92% 50%;      /* 决策 D16：amber，与 --color-cta 视觉等价但语义独立 */
}

.dark {
  --color-warning-hsl: 38 75% 55%;      /* dark 模式独立调：饱和度↓ 亮度略↑，避免刺眼 */
}

/* 2. Tailwind v4 @theme 段注册为 color token，让 bg-warning/text-warning/border-warning utility 生效 */
@theme {
  --color-warning: hsl(var(--color-warning-hsl));
}

/* 3. Radix Collapsible keyframes（F1 修订：FieldCard 真正的 200ms 折叠动画依赖此处）*/
...
**Tailwind v3 的项目改用 tailwind.config.ts**（若 m2c1 globals.css 没用 `@theme` 段就走这里）：
...
**实施期决策点**：先看 `frontend/src/styles/globals.css` 是否含 `@theme` block——有则走方案 1（v4），否则走方案 2（v3）。两者只取一种。

- [ ] **Step 3: 首次引入 shadcn Collapsible（F1 + F4 联合修订）**

```bash
pnpm --dir frontend dlx shadcn@latest add collapsible
```

按 m2c1 §3.5.7 红线 7 项复审 `frontend/src/components/ui/collapsible.tsx`：

- [ ] 移除 `"use client"` 残留
- [ ] focus-visible 与 §3.5.2 "显眼方块框" 对齐（默认 Radix 已提供，验证不被 shadcn override 弱化）
- [ ] no hover translateY / no transform: scale
- [ ] no shadow-lg 等大阴影
- [ ] 同 PR 内完成
- [ ] grep `frontend/src/components/ui/collapsible.tsx` 含 `data-[state=open]:animate-collapsible-down` 类（与 globals.css keyframes 配对）；若无，手动加到组件
- [ ] tsc 不退化
...
Expected: 全绿（无新测，仅 schema + token + Collapsible 引入）。

- [ ] **Step 5: 提交**

```bash
git add frontend/src/api/generated/schema.d.ts frontend/src/styles/globals.css frontend/src/components/ui/collapsible.tsx
git commit -m "chore(types-ui): gen:api PATCH + --color-warning token（CSS var + Tailwind utility 双层）+ 引入 shadcn Collapsible + collapsible-up/down keyframes（M2c-4 PR-3 Task 18）"
```
...
- [ ] **Step 1: 实现组件（F7 修订：a11y——X 按钮 keyboard focus 也要可见）**
...
**a11y 关键**：`focus-visible:opacity-100` 让键盘 Tab 到 X 按钮时也显形（不仅 hover）；`focus-visible:ring-2 focus-visible:ring-ring` 让焦点态可见，符合 m2c1 §3.5.5 "Focus states must be visible" + §3.5.2 "显眼方块框"。
...
- [ ] **Step 0: m2c1 §3.5.7 红线复审 shadcn Select（F9 修订）**

shadcn Select 在 M2c-3 已引入，但本里程碑首次密集使用——按 m2c1 §3.5.7 复审 4 项：

```bash
# 1. no use client 残留
grep "use client" frontend/src/components/ui/select.tsx
```
Expected: 0 命中（M2c-3 已清理；若有则同 PR 内删）

```bash
# 2. focus ring 已重调到 §3.5.2 "显眼方块框"
grep "focus-visible:ring-2 focus-visible:ring-ring" frontend/src/components/ui/select.tsx
```
Expected: 命中（M2c-3 已调好）

```bash
# 3. SelectContent 用 bg-popover token，未硬编码 bg-white
grep -n "bg-popover\|bg-white" frontend/src/components/ui/select.tsx
```
Expected: `bg-popover` 命中、`bg-white` 0 命中（M2c-3 已修）

- [ ] 4. SelectTrigger chevron icon = `ChevronsUpDown`（Lucide，沿用 M2c-3 Combobox），不是 Select 默认的下三角 / ChevronDown。若 shadcn Select 默认是 ChevronDown，手改为 ChevronsUpDown 与 Combobox 视觉一致
...
### Task 25: field-card（折叠/展开，用 Radix Collapsible）

**Files:**
- Create: `frontend/src/features/types/form/custom-fields-builder/field-card.tsx`

**Pre-Step**：依赖 Task 18 已引入 shadcn Collapsible + globals.css 注册了 `animate-collapsible-down/up` keyframes（F1 修订）。如未做请回 Task 18 完成。

**Pre-Step 2 · m2c1 §3.5.7 首次引入 shadcn Card 红线复审（F4 修订）**：
...
复审清单（同 PR 完成）：
- [ ] 移除 `"use client"` 残留
- [ ] 默认 variant 的 `shadow-sm` 与 m2c1 §3.5 "不堆 shadow" 对照——若 Card 默认含 hover translateY / shadow-md→shadow-lg 升级，删除（与 m2c2 实施期 §1 决议一致）
- [ ] padding 与 spec §8.1 "卡片折叠态高度固定（≤ 56px）" 协调（默认 padding 太大会让单行 56px 不够，本组件直接覆盖 className 不依赖默认 padding）
- [ ] focus-visible 沿用 m2c1 全局 `*:focus-visible` 兜底
- [ ] no `bg-card` 硬编码（用 token）
- [ ] no `transition-shadow` hover（避免 AI-slop hover-translateY）
- [ ] 同 PR 内完成审查
...
**关键修订**：原来的 `transition-[height]` 在 `auto → auto` 上不工作（CSS 已知限制）；改用 Radix Collapsible 的 `--radix-collapsible-content-height` CSS 变量驱动 keyframes，是 200ms 动画**真正生效**的方法。文件顶部 import 加：
...
- [ ] **Step 1: 实现 skeleton（F8 修订：复用 shadcn Skeleton 组件，不重造轮子）**
...
**为什么复用 Skeleton 而不是裸 div**：M2c-2 已引入 shadcn `<Skeleton>`（`frontend/src/components/ui/skeleton.tsx`），与 GridSkeleton / DetailSkeleton / TimelineSkeleton 共用同一抽象——视觉一致性 > 自由度。`animate-pulse` 是 Tailwind 内置不在红线（红线禁 `animate-spin`），但通过 Skeleton 组件复用可以让未来"全局换 loading 视觉"时只改一处。

- [ ] **Step 2: 实现 types-table（F5 修订：用 @tanstack/react-table 范式与 assets-table 同源，密度/行高/hover 不漂移）**

**约束**：spec §8.1 要求 "与 assets-table 完全同款（密度、行高、列分隔、空态图标）"。assets-table 用 `@tanstack/react-table` (M2c-1 已引入) 的 `useReactTable` + `flexRender` 范式。本文件**保持同范式**，行高/padding/hover token **从 assets-table.tsx 拷贝常量**，避免漂移。
...
function RefCountCell({ typeId }: { typeId: string }) {
  ...
  // F12 修订：用 Skeleton 占位而非 "…" 字符串，与全局 loading 抽象一致
...
**实施期检查**：打开 `frontend/src/features/assets/list/assets-table.tsx`，对照本组件的 `border-border` / `text-xs uppercase tracking-wide text-muted-foreground` / `px-3 py-2` / `hover:bg-muted/50` 是否一致；不一致就以 assets-table 为准修。
...
export function TypeDetailPage({ id }: { id: string }) {
  ...
  if (q.isError) {
    // F3 修订：区分 404 vs 服务器/网络错误，404 走专门的 not-found 状态
  ...
  if (!q.data) return null; // 理论 race condition 兜底（query 成功但 data 为 undefined）
...
## frontend-design Plan Review 修订（spec §8.3 ③）

应用 9 项修订（HIGH 3 + MEDIUM 6，LOW 3 待 release notes followup）：

| ID | 严重度 | 修订内容 | 落点 |
|---|---|---|---|
| F1 | HIGH | FieldCard 折叠动画用 Radix Collapsible（`transition-[height]` 在 auto→auto 不工作） | Task 18 + Task 25 |
| F2 | HIGH | `--color-warning` 同时注册 CSS variable + Tailwind utility（`@theme` 段或 tailwind.config） + 加 collapsible-down/up keyframes | Task 18 |
| F3 | HIGH | TypeDetailPage 区分 404 vs 服务器错误，404 走专门 not-found 状态 | Task 31 |
| F4 | MEDIUM | shadcn Card 首次引入按 m2c1 §3.5.7 红线复审 7 项 | Task 25 Pre-Step 2 |
| F5 | MEDIUM | types-table 改用 @tanstack/react-table 范式与 assets-table 同源 | Task 29 Step 2 |
| F6 | MEDIUM | 卡片间距 `space-y-2` → `space-y-4`（var(--space-md) = 16px） | Task 26 |
| F7 | MEDIUM | chip X 按钮加 `focus-visible:opacity-100` + ring，键盘可达 | Task 22 |
| F8 | MEDIUM | TypesTableSkeleton 复用 shadcn `<Skeleton>` 组件 | Task 29 Step 1 |
| F9 | MEDIUM | field-type-selector 按 m2c1 §3.5.7 红线 4 项复审 shadcn Select | Task 23 Pre-Step 0 |

LOW 3 项（heading 文案 / RefCountCell loading）部分已合并 F12 复用 Skeleton 解决；其余微优化（heading 简化）登记到 release notes followup。
