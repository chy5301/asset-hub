...
export function CheckoutDialog({ assetId, trigger }: Props) {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      toast({ title: "派发失败", description: String(e), variant: "destructive" });
    }
  };
...
export function ReturnDialog({ assetId, trigger }: Props) {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      toast({ title: "归还失败", description: String(e), variant: "destructive" });
    }
  };
...
export function SimpleTransitionDialog({ assetId, kind, trigger }: Props) {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      toast({ title: `${meta.label}失败`, description: String(e), variant: "destructive" });
    }
  };
...
export function RetireAlertDialog({ assetId, assetName, trigger }: Props) {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      toast({ title: "退役失败", description: String(e), variant: "destructive" });
    }
  };
...
export function DisposeAlertDialog({ assetId, assetName, trigger }: Props) {
  ...
  const onConfirm = async () => {
    try {
      ...
      toast({ title: "处置失败", description: String(e), variant: "destructive" });
    }
  };
...
export function RelocateDialog({ assetId, trigger }: Props) {
  ...
  const onSubmit = async (v: FormValues) => {
    try {
      ...
      toast({ title: "变更位置失败", description: String(e), variant: "destructive" });
    }
  };
...
export function TransferHolderDialog({ assetId, trigger }: Props) {
  ...
  const onSubmit = async (v: FormValues) => {
    try {
      ...
      toast({ title: "变更保管人失败", description: String(e), variant: "destructive" });
    }
  };
...
import { TimelineSkeleton } from "@/components/feedback/skeleton";  // 实际路径按现有
import type { components } from "@/api/generated/schema";
        ...
        return (
          ...
          >
            <span
              ...
              style={{
                backgroundColor: `color-mix(in oklch, var(${meta.bgVar}) 15%, transparent)`,
...
- [ ] **Step 16.1: simplify §J/§L 重构 build-asset-schema**

把 `frontend/src/features/assets/form/build-asset-schema.ts` 的双函数（create / edit）合一：

- 用单一 `buildAssetSchema(fieldDefs, mode: "create" | "edit")` 接受 mode 参数
- mode 决定 `type_id` 是 required（create）还是 optional（edit），通过显式 `if (mode === 'create')` 分支构造而非条件 `.extend`
- 输出类型 `AssetFormValues` 推导清晰，不再丢 `type_id`

修改 `asset-create-form.tsx`：
- 删除 `as unknown as Resolver<AssetFormValues>` cast（共 2 处）
- 删除任何 `as never` 等 zod-related cast

修改 `type-form.tsx`：
- 删除 M2c-4 PR-3 Task 27 引入的 `as unknown as Resolver<>` cast
