...
> **M2c-4 已落地（PR-2，feature/m2c4-form-infra，commit db9f946）**：Task 10 合并为 `buildAssetSchema(fieldDefs, { mode: 'create' | 'edit' })`，Task 11 迁移 asset-create/edit-form 调用点。
...
> **M2c-4 已落地（PR-2，feature/m2c4-form-infra，commit db9f946）**：Task 13 抽 `<FieldShell>` 组件（含 `layout="inline"` prop 处理 bool-field 特例），Task 14 迁 8 个 field-controls 使用 FieldShell。
...
> **M2c-4 已落地（PR-2，feature/m2c4-form-infra，commit db9f946）**：Task 15 把 `AssetFormFields` 与各 field-controls 改泛型 `Control<TFieldValues>`，消除 `as unknown as Control` 双重 cast。
...
> **M2c-4 已落地（PR-2，feature/m2c4-form-infra，commit db9f946）**：Task 10 合并 schema builder 时一并把 `CREATE_EMPTY_SCHEMA` 提取为 module-level 常量，消除每次 render 重建。
...
> **🔴 待处理（高优先）**：M3 排期前**必须**修复，属于运行时数据丢失类 bug。PR-3 范围内未触碰（spec 明确不在 scope）。
...
> **已修复（M2c-4 PR-3 Task 35，commit 77caf01）**：`useMatchRoute` 的 `fuzzy: true` 改为精确匹配（`fuzzy: false`），同时把 NavBar `/` 路径恢复为真实 `to: '/'`，`/types` 注册真实路由后激活态不再同时 active。
...
**何时做**：✅ 已在 PR-3 T35（commit 77caf01）修复，属于 PR-3 必做项已闭环。

**ROI**：中。修复后激活态视觉不再错乱；属于 PR-3 必做项。

---

### Q · `TypeRead` 缺 `ref_count` 字段 → 前端 N+1 query

> **⏳ 待处理（M3 周期）**：M2c-4 PR-3 final review 发现。

**位置**：`frontend/src/features/types/list/types-table.tsx:25-30`（`RefCountCell` 每行 `useAssetsQuery` 拉一次）+ `frontend/src/features/types/detail/type-delete-dialog.tsx`（删除前再查一次）。

**现状**：`TypeRead` schema 不含 `ref_count`，前端只能通过 `useAssetsQuery({ type })` 反向取列表长度推算。types-table 50 行 type 即 50 个并发 query；delete dialog 用 `pageSize: 200` 兜上限（commit d3099cd 修了显示 0/1 的 bug，但仍依赖列表 length）。

**候选**：
- 后端 `AssetTypeService.list_types` 返回时 join 一次 `select count(*) from assets group by type_id`，`TypeRead` 加 `ref_count: int` 字段
- 或 service 层 `get_type` 时 lazy 加（detail 页）+ `list_types` batch 加（list 页）
- 前端 `RefCountCell` 直接读 `type.ref_count`，删 N+1 query；`TypeDeleteDialog` 也走同字段，删 `useAssetsQuery` 调用

**何时做**：M3 周期。当前 v1 单作者数据量小，N+1 不构成性能问题；但 list 上限隐患（>200 个 asset 引用同 type 时，delete dialog 显示数会截断）真实存在。

**ROI**：中。消除 N+1 + 真值 ref_count + 简化 delete dialog 两个守卫合并为一个。
