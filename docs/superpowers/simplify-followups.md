...
- **PR-3**（feature/m2c4-types-ui，2026-05-02 合并）：类型管理 web UI（list/detail/create/delete）+ custom_fields 结构化 builder + unknown-key banner + §P NavBar fuzzy match 修复

§6 记录的所有 M2c-4 范围 follow-up 条目（J/K/L/M/N/O/P）均已处理：J/L 条件 .extend cast 保留（已加注释 + 属于预知 trade-off）；K（acquired_at bug）M3 排期前独立修；N/O M3 加新 field-control 时一并做；P 已修复。
