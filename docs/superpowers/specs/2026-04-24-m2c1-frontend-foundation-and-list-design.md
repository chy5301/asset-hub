...
| 允许 / 禁止 | 字体 | 备注 |
| --- | --- | --- |
| **✓ 允许（主用）** | Fira Code（Variable，heading + mono） | 技术感、非通用、与 MASTER 一致 |
| **✓ 允许（主用）** | Fira Sans（Variable，body） | 同上 |
...
```jsonc
"dependencies": {
  "@fontsource-variable/fira-code": "^5.x",   // 新增：§3.5.3
  "@fontsource-variable/fira-sans": "^5.x",   // 新增：§3.5.3
...
- Fallback 链（英文）：`"Fira Sans Variable", "Fira Sans"`
- Fallback 链（中文，有意选择而非 `system-ui` 兜底）：`"PingFang SC", "Microsoft YaHei UI", sans-serif`
- **完整 body font-family**：`"Fira Sans Variable", "Fira Sans", "PingFang SC", "Microsoft YaHei UI", sans-serif`
- **Heading / mono** 统一：`"Fira Code Variable", "Fira Code", ui-monospace, monospace`
...
13. **字体审计（§3.5.3）**：DevTools Computed → Font Family，页面任意 body 文字渲染字体应为 `Fira Sans Variable`，编号字段为 `Fira Code Variable`；**若看到 Inter / Roboto / system-ui / Geist 视为失败**
